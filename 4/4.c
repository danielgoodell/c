...
int main()
{
	...
	while ((type = getop(s)) != EOF) {
		...
		case NUMBER:
			push(atof(s));
			break;
		case 'c':			//added c command to clear the stack
			clear_stack();
			break;
		case 'p':			//added p command to print the current stack value without popping it
			print_top();
			getop(s);
			break;
		case 's':			//added s command to swap the top two elements on the stack.
			op2 = pop();
			op3 = pop();
			push(op2);
			push(op3);	
